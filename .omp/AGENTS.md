...
机械操作（typo、import 整理、纯重构不涉逻辑变更）可在 verify 后直接 commit，跳过 understand→research→design→document。
涉及逻辑判断的修复不可豁免。
## 摩擦记录
