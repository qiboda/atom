...
// impl From<&mut TerrainGlobalCoord> for VoxelGradedCoord {
//     fn from(value: &mut TerrainGlobalCoord) -> Self {
//         Self {
//             chunk_coord: TerrainChunkCoord::from(value),
//             voxel_coord: TerrainVoxelCoord::from(value),
//         }
...
/// global coord to chunk coord
impl From<&TerrainChunkCoord> for TerrainChunkCoord {
    fn from(value: &TerrainChunkCoord) -> Self {
        Self {
            x: value.x,
            y: value.y,
            z: value.z,
...
impl From<&TerrainGlobalCoord> for TerrainVoxelCoord {
    fn from(value: &TerrainGlobalCoord) -> Self {
        Self {
            x: value.x as usize % TERRAIN_VOXEL_NUM_IN_CHUNK,
            y: value.y as usize % TERRAIN_VOXEL_NUM_IN_CHUNK,
            z: value.z as usize % TERRAIN_VOXEL_NUM_IN_CHUNK,
        }
    }
}
...
#[cfg(test)]
mod coord_test {
    use super::{TerrainGlobalCoord, TerrainVoxelCoord};

    #[test]
    fn test_global_to_voxel() {
        let voxel_coord = TerrainVoxelCoord::from(&TerrainGlobalCoord {
            x: -32,
            y: 0,
            z: -47,
        });

        assert_eq!(voxel_coord.z, 1);
        assert_eq!(voxel_coord.y, 0);
    }
}
