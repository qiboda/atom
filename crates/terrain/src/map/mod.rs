...
pub fn generate_area(
    ...
    let mut area_id = 0;
    for area_setting in map_config.rand_area_setting.iter() {
        let num = map_context
            .rng
            .gen_range(area_setting.rand_area_num.clone());
        for _ in 0..num {
            loop {
                let rng_range = area_setting.rand_area_range_percent.clone();
                let x = map_context.rng.gen_range(
                    rng_range.start.x * map_config.grid_num as f32
                        ..rng_range.end.x * map_config.grid_num as f32,
                ) as usize;
                let y = map_context.rng.gen_range(
                    rng_range.start.y * map_config.grid_num as f32
                        ..rng_range.end.y * map_config.grid_num as f32,
                ) as usize;
                // TODO 统一列主序还是行主序
                ...
                map.sites_info[index].area_id = area_id;
                area_id += 1;

                map.sites_info[index].area_weight = 1.0;
                map.sites_info[index].terrain_type = Some(MapFlatTerrainType::PlainGrassLand);
                parent_sites.push(index);

                map.area_random_points.push(index);

                let radius_rng_range = area_setting.rand_area_radius.clone();
                let r = map_context.rng.gen_range(radius_rng_range);
                declines.push(1.0 / r as f64);
    ...
    while !parent_sites.is_empty() {
        ...
        for parent_index in parent_sites.iter() {
            ...
            let decline = declines[parent_site_info.area_id];

            let adjust = map_context.rng.gen_range(
            ...
            for neighbor_index in map.diagram.neighbors[*parent_index].clone() {
                ...
                neighbor_site_info.terrain_type = parent_site_info.terrain_type;
...
pub fn generate_heights(
    ...
    let num = map_context
        .rng
        .gen_range(map_config.rand_height_setting.rand_point_num.clone());

    let mut count = 0;
    loop {
        let x = map_context.rng.gen_range(0..map_config.grid_num);
        let y = map_context.rng.gen_range(0..map_config.grid_num);
        // TODO 统一列主序还是行主序
        let index = x * map_config.grid_num + y;
        ...
        let height = map_context
            .rng
            .gen_range(map_config.rand_height_setting.rand_point_height.clone());
        let radius = map_context
            .rng
            .gen_range(map_config.rand_height_setting.rand_point_radius.clone());

        map.sites_info[index].height = height;
        map.sites_info[index].height_id = count;
        map.height_random_points.push(index);
        parent_sites.push(index);
        height_declines.push(1.0 / radius as f64);
    ...
    while !parent_sites.is_empty() {
        ...
        for parent_index in parent_sites.iter() {
            let parent_site_info = map.sites_info[*parent_index];
            if parent_site_info.height < 0.0 {
                continue;
            }

            let height_decline = height_declines[parent_site_info.height_id];

            let height_adjust = map_context.rng.gen_range(
                -height_decline..(height_decline * 0.5),
            ...
            for neighbor_index in map.diagram.neighbors[*parent_index].clone() {
                ...
                neighbor_site_info.height =
                    parent_site_info.height - height_decline + height_adjust;
                neighbor_site_info.height_id = parent_site_info.height_id;

                if neighbor_site_info.height.floor() != parent_site_info.height.floor() {
                    neighbor_site_info.slope = true;
                }
...
//         island_data.extend(map.area_random_points.clone());
//         island_data.swap();
...
pub fn generate_map_image(
    ...
    let mut terrain_map_image: ImageBuffer<image::Rgba<f32>, Vec<f32>> = image::Rgba32FImage::new(
    ...
    for triangle_indices in map.diagram.delaunay.triangles.chunks_exact(3) {
        ...
        let slope_0 = map.sites_info[triangle_indices[0]].slope as u32 as f32;
        let slope_1 = map.sites_info[triangle_indices[1]].slope as u32 as f32;
        let slope_2 = map.sites_info[triangle_indices[2]].slope as u32 as f32;
        ...
        for point in points {
            ...
            let slope = triangle_interpolation(
                point.as_vec2(),
                pt0.as_vec2(),
                pt1.as_vec2(),
                pt2.as_vec2(),
            ...
            terrain_map_image.put_pixel(
                point.x,
                point.y,
                image::Rgba([height, humidity, temperature, slope]),
            );
    ...
    let mut image = Image::from_dynamic(
        image::DynamicImage::ImageRgba32F(terrain_map_image),
        false,
...
pub fn generate_biome_image(
    ...
    for (i, cell) in map.diagram.cells.iter().enumerate() {
        let terrain_type = map.sites_info[i]
            .terrain_type
            .unwrap_or(MapFlatTerrainType::Ocean);
        let flat_terrain_type: MapFlatTerrainType = terrain_type;
...
pub fn draw_area_image(map: Res<TerrainMap>, map_setting: Res<config::TerrainMapSetting>) {
    ...
    for (i, cell) in map.diagram.cells.iter().enumerate() {
        let area_id = map.sites_info[i].area_id;
        let color = if area_id == usize::MAX {
            image::Rgba([255, 255, 255, 255])
        } else {
            match area_id % 4 {
                0 => image::Rgba([255u8, 0, 0, 255]),
                1 => image::Rgba([0, 255, 0, 255]),
                2 => image::Rgba([0, 0, 255, 255]),
                3 => image::Rgba([125, 200, 200, 255]),
                _ => panic!("area id too big!"),
            }
        };
        let points = cell
            .points()
        ...
        for neighbor in neighbors {
            if let Some([p0, p1]) = shared_edge(cell, &map.diagram.cells[neighbor]) {
                ...
                if map.sites_info[neighbor].height.floor() != map.sites_info[i].height.floor() {
                    let max_height = map.sites_info[neighbor]
                        .height
                        .floor()
                        .max(map.sites_info[i].height.floor());
    ...
    image
        .save(map_setting.image_save_path.join("terrain_area.png"))
        .unwrap();
}
