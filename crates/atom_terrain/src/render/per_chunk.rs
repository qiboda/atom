...
pub fn per_chunk_render_system(
    ...
    let solid_id = if debug_config.show_chunk_bounds {
        if debug_config.double_sided {
            render_pipeline.tint_double
        } else {
            render_pipeline.tint_culled
        }
    } else {
        if debug_config.double_sided {
            render_pipeline.solid_double
        } else {
            render_pipeline.solid_culled
        }
    };
    ...
    let Some(solid_pso) = pipeline_cache.get_render_pipeline(solid_id) else {
        return;
    };
    ...
    for (&_cid, &slot_idx) in &manager.active {
        let Some(slot) = &compute_pipeline.slots[slot_idx] else {
            continue;
        };
        if slot.pass < 6 {
            continue;
    ...
    if debug_config.wireframe {
        let wire_id = if debug_config.double_sided {
            render_pipeline.wire_double
        } else {
            render_pipeline.wire_culled
        };
