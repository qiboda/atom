...
fn write_average_normal_to_tangent(mesh: &mut Mesh) {
    let positions = mesh
        ...
        .expect("`Mesh::ATTRIBUTE_POSITION` vertex attributes should be present")
        .as_float3()
        .expect("`Mesh::ATTRIBUTE_POSITION` vertex attributes should be of type `float3`");

    let normals = mesh
        .attribute(Mesh::ATTRIBUTE_NORMAL)
        .expect("`Mesh::ATTRIBUTE_NORMAL` vertex attributes should be present")
        .as_float3()
    ...
    for vertex in positions.iter() {
        if let Some(vertex_index) = vertices.iter().position(|x| *x == vertex) {
            let normal = avg_normals[vertex_index];
            tangent.push([normal[0], normal[1], normal[2], 0.0]);
        } else {
            tangent.push([0.0, 0.0, 0.0, 0.0]);
            warn!("Cannot find average normal for vertex: {:?}", vertex);
        }
    }
