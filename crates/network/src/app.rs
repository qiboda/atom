...
impl Apps {
    ...
    pub fn new<T: BuildApp>(settings: Settings, cli: Cli) -> Self {
        match cli {
            #[cfg(not(target_family = "wasm"))]
            Cli::HostServer { client_id } => {
                ...
                let app = T::combined_app(settings.clone());
                let (client_config, server_config) =
                    combine_config(settings, vec![], client_net_config);
                Apps::HostServer {
                    app,
                    client_config,
                    server_config,
            ...
            Cli::ClientAndServer { client_id } => {
                ...
                let client_app = T::client_app(settings.clone());
                let client_config = client_config(settings.clone(), net_config);
                ...
                let server_app = T::server_app(settings.clone());
            ...
            #[cfg(not(target_family = "wasm"))]
            Cli::Server => {
                let app = T::server_app(settings.clone());
                let config = server_config(settings, vec![]);
            ...
            Cli::Client { client_id } => {
                ...
                let app = T::client_app(settings.clone());
                let net_config = get_client_net_config(&settings, client_id);
                let config = client_config(settings, net_config);
...
fn client_config(settings: Settings, net_config: client::NetConfig) -> ClientConfig {
    ClientConfig {
...
pub struct DefaultBuildApp;

impl BuildApp for DefaultBuildApp {
