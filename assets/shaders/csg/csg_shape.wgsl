...
fn sd_box_frame(p: vec3f, b: vec3f, e: f32) -> f32 {
    ...
    return min(
        min(
            length(max(vec3f(lp.x, q.y, q.z), vec3f(0.0))) + min(max(lp.x, max(q.y, q.z)), 0.0),
            length(max(vec3f(q.x, lp.y, q.z), vec3f(0.0))) + min(max(q.x, max(lp.y, q.z)), 0.0)
        ),
        length(max(vec3f(q.x, q.y, lp.z), vec3f(0.0))) + min(max(q.x, max(q.y, lp.z)), 0.0)
    );
}
...
fn sd_round_cone(p: vec3f, a: vec3f, b: vec3f, r1: f32, r2: f32) -> f32 {
    ...
    let y = dot(pa, ba);
    let z = y - l2;
    let dot_args = pa * l2 - ba * y;
    let x2 = dot(dot_args, dot_args);
    let y2 = y * y * l2;
    let z2 = z * z * l2;
...
fn sd_rhombus(p: vec3f, la: f32, lb: f32, h: f32, ra: f32) -> f32 {
    ...
    let f = clamp((la*pp.x - lb*pp.z + lb* lb) / (la * la + lb * lb), -1.0, 1.0);
    let w = pp.xz - b * vec2(f, 1.0 - f);
    let q = vec2(length(w) * sign(w.x) - ra, pp.y - h);
...
fn sd_triangle(p: vec3f, a: vec3f, b: vec3f, c: vec3f) -> f32 {
    ...
    let dot_arg1 = ba * clamp(dot(ba, pa) / dot(ba, ba), 0.0, 1.0) - pa;
    let dot_arg2 = cb * clamp(dot(cb, pb) / dot(cb, cb), 0.0, 1.0) - pb;
    let dot_args = ac * clamp(dot(ac, pc) / dot(ac, ac), 0.0, 1.0) - pc;

    return sqrt(
        select(
            min(min(
                dot(dot_arg1, dot_arg1),
                dot(dot_arg2, dot_arg2)
            ),
                dot(dot_args, dot_args)),
...
fn sd_quad(p: vec3f, a: vec3f, b: vec3f, c: vec3f, d: vec3f) -> f32 {
    ...
    let dot_arg1 = ba * clamp(dot(ba, pa) / dot(ba, ba), 0.0, 1.0) - pa;
    let dot_arg2 = cb * clamp(dot(cb, pb) / dot(cb, cb), 0.0, 1.0) - pb;
    let dot_arg3 = dc * clamp(dot(dc, pc) / dot(dc, dc), 0.0, 1.0) - pc;
    let dot_arg4 = ad * clamp(dot(ad, pd) / dot(ad, ad), 0.0, 1.0) - pd;
        ...
        select(
            ...
            min(
                min(
                    ...
                    dot(dot_arg3, dot_arg3)
                ),
                dot(dot_arg4, dot_arg4)
            ),
