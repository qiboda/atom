...
fn get_terrain_noise(location: vec3f) -> f32 {
    let density_value = plane(location, vec3f(0.0, 1.0, 0.0), 2.0);
    // let loc = location + vec3f(6.0, 6.0, 6.0);
    // let density_value = cube(loc, vec3f(14.0, 14.0, 14.0));
    // let density_value = location.y - (open_simplex_2d_fbm_with_seed(location.xz, 323u, 3u, 0.0025, 0.1, 1.0) + 0.8) * 100.0;
