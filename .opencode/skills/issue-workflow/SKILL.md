...
管理 atom 项目的完整 GitHub issue 生命周期。处理
单 issue 创建（从 atom-workflow 门禁第 2 步委派而来）、
...
- atom-workflow 预实现门禁第 2 步自动化（通过 `→ 调用 /issue-workflow`）
...
| 模式 | 触发条件 | 流程 |
|---|---|---|
| **单 issue** | atom-workflow 门禁第 2 步，单一 issue 需求 | 创建一个 issue → 展示 URL → 完成 |
| **Epic + 子 issues** | 计划产生 2+ 任务批次且各批次有独立交付物 | 创建 epic → 批量创建子 issues → 计划中跟踪 → 批次关闭 |

## 工作流

### 阶段 0：确定模式

1. 阅读 grill-me 总结和计划输出（`.omo/plans/<name>.md` 或对话中的任务分解）
...
1. 首先创建 **epic**（父 issue）：
   ...
   ```
   Epic body 包含：动机、范围概述、指向计划文件的链接。

2. 用 `gh issue view <epic-N>` 验证

### 阶段 1B：Epic 模式 — 创建子 issues

1. 从计划中提取每个任务批次中属于独立交付物的项目。
...
计划文件是权威的跟踪文档。其 `## Tasks` 章节使用 Markdown 表格：
...
在包含所有子 issue commit 的 PR 合并到 `main` 之后：
...
```
## Issue: #<N> — <title>
URL: https://github.com/qiboda/atom/issues/<N>
Labels: <labels>
```
...
URL: https://github.com/qiboda/atom/issues/<epic-N>
...
- **PR 合并前自动关闭 issues**——仅在合并到 `main` 后才关闭
- **跳过批次确认**——除非用户明确要求自动模式
- **删除计划文件条目**——只更新状态，绝不删除行
- **创建无父 issue 的子 issues**——始终使用 `--parent` 标志
- **在 commit 中使用 `fixes #N` 或 `closes #N`**——只用 `ref #N`（通过 `gh issue close` 手动关闭）
- **修改 atom-workflow skill**——issue-workflow 是同级协作，非替代
- **为非 feature/bugfix 工作创建 issues**——文档、lint、typo 修复完全跳过 issue 创建

## 与 atom-workflow 的协作

1. atom-workflow 门禁第 2 步显示 `→ 调用 /issue-workflow 创建/管理 issues`
2. 在门禁第 2 步，atom-workflow 调用 issue-workflow 创建 issue(s)
3. issue-workflow 决定单 issue 还是 epic 模式，创建 issues，返回结果
4. atom-workflow 对每个子 issue 独立执行门禁步骤 3-5c
5. 所有子 issues 完成后 → PR 合并 → issue-workflow 处理批次关闭
6. atom-workflow 处理 commit、test、review 和 push（不变）

## 参考资料

- `AGENTS.md` — 完整项目工作流和门禁规则
- `.opencode/kb/github/labels.md` — 标签分类体系
